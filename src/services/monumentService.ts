...
const LOCAL_STORAGE_KEY = 'mt_red_sites_data_v8';
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const mapped: RedSite[] = data.map((item: any) => ({
          ...
          toaDo: {
            lat: Number(item.latitude || item.lat || item.toaDo?.lat || 10.748),
            lng: Number(item.longitude || item.lng || item.toaDo?.lng || 106.650),
          },
  ...
  if (supabase) {
    try {
      const payload = {
        ...
        latitude: site.toaDo.lat,
        longitude: site.toaDo.lng,
